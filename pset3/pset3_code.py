...
def ComputeAccuracy(test_set, test_set_predicted):
    ...
    accuracy_tagging = (1 - float(mistake_counter)/(total_counter - 2*len(test_set)))*100
    accuracy_sentence = (1 - float(lineMistake_counter)/len(test_set))*100
...
def main():
    ...
    for i in range(len(training_set_prep)):
        try:
    	   total_logprob += log(bigram_hmm.JointProbability(training_set_prep[i]))
        except:
            pass

    print "Sanity Check  ========>>>>", total_logprob

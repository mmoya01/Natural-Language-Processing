...
def main():
	...
	print "Total No. of production with NONTERMINAL (S) " + str(len(grammar.productions()))

	print '-------------------------------------------------------------------------------'

	print " MOST PROBABLE 10 PRODUCTIONS ----"
	for i in range(0,10):
	...
	# Bucket1
	f_test_1 = open("test_1e", "w")
	f_gold_1 = open("gold_1e", "w")
	for tree in bucket1:	
		sent = tree.leaves()
	...
	# Bucket2
	f_test_2 = open("test_2e", "w")
	f_gold_2 = open("gold_2e", "w")
	for tree in bucket2:	
		sent = tree.leaves()
		parse_result = invertedGrammar.Parse(sent)
		back_pointer = parse_result[1]
	...
	# Bucket3
	f_test_3 = open("test_3e", "w")
	f_gold_3 = open("gold_3e", "w")
	for tree in bucket3:	
		sent = tree.leaves()
		parse_result = invertedGrammar.Parse(sent)
		back_pointer = parse_result[1]
	...
	# Bucket4
	f_test_4 = open("test_4e", "w")
	f_gold_4 = open("gold_4e", "w")
	for tree in bucket4:	
		sent = tree.leaves()
		parse_result = invertedGrammar.Parse(sent)
		back_pointer = parse_result[1]
	...
	# Bucket5
	f_test_5 = open("test_5e", "w")
	f_gold_5 = open("gold_5e", "w")
	for tree in bucket5:	
		sent = tree.leaves()
		parse_result = invertedGrammar.Parse(sent)
		back_pointer = parse_result[1]
